    ...
    Win32::{
        ...
        UI::{
            Input::KeyboardAndMouse::{
                keybd_event, RegisterHotKey, UnregisterHotKey, HOT_KEY_MODIFIERS,
                KEYBD_EVENT_FLAGS, KEYEVENTF_KEYUP, MOD_ALT, MOD_CONTROL, MOD_NOREPEAT,
                MOD_SHIFT, MOD_WIN, VK_CONTROL,
            },
            WindowsAndMessaging::{
                CreateWindowExW, DefWindowProcW, DispatchMessageW, GetMessageW, PostMessageW,
                RegisterClassW, SetForegroundWindow, TranslateMessage, GetForegroundWindow,
...
const CLIPBOARD_SHORTCUT_VERSION: u8 = 1;
...
static CLIPBOARD_PASTE_TARGET: OnceLock<Mutex<Option<isize>>> = OnceLock::new();
...
impl Default for ClipboardHistorySettings {
    fn default() -> Self {
        Self {
            ...
            shortcut_version: CLIPBOARD_SHORTCUT_VERSION,
        }
    }
}
...
impl ClipboardHistorySettings {
    fn normalized(mut self) -> Self {
        ...
        self.shortcut = parse_shortcut_binding(&self.shortcut)
            ...
            .label;
        if self.shortcut_version < CLIPBOARD_SHORTCUT_VERSION {
            if self.shortcut == "Ctrl+X" {
                self.shortcut = DEFAULT_SHORTCUT.to_string();
            }
            self.shortcut_version = CLIPBOARD_SHORTCUT_VERSION;
        }
        self
    }
}
...
#[tauri::command]
pub fn copy_clipboard_history_item_and_paste_back(
    id: String,
) -> Result<ClipboardHistorySnapshot, String> {
    service()?.copy_item_and_paste_back(&id)
}
...
impl ClipboardHistoryService {
    ...
    fn copy_item_and_paste_back(&self, id: &str) -> Result<ClipboardHistorySnapshot, String> {
        let snapshot = self.copy_item(id)?;
        let target = CLIPBOARD_PASTE_TARGET
    ...
    fn emit_shortcut(&self) {
        ...
        if !target.0.is_null() {
            if let Ok(mut stored_target) = CLIPBOARD_PASTE_TARGET
                .get_or_init(|| Mutex::new(None))
                .lock()
            {
                *stored_target = Some(target.0 as isize);
            }
        }

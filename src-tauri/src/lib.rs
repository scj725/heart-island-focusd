...
const COLLAPSED_ISLAND_WIDTH: f64 = 640.0;
...
#[tauri::command]
async fn get_qq_music_lyrics(title: String, artist: String) -> Result<String, String> {
    tauri::async_runtime::spawn_blocking(move || fetch_qq_music_lyrics(&title, &artist))
        .await
        .map_err(|error| format!("Failed to join the QQ Music lyrics task: {error}"))?
}
...
fn normalized_media_text(value: &str) -> String {
    value
        .to_lowercase()
        .chars()
...
fn fetch_qq_music_lyrics(title: &str, artist: &str) -> Result<String, String> {
    ...
    let client = reqwest::blocking::Client::builder()
        ...
        .build()
        .map_err(|error| format!("Failed to create QQ Music request client: {error}"))?;
    ...
    .map_err(|error| format!("Failed to create QQ Music search URL: {error}"))?;
    let search: Value = client
        .get(search_url)
        .header("Referer", "https://y.qq.com/")
        .send()
        .and_then(|response| response.error_for_status())
        .map_err(|error| format!("QQ Music search failed: {error}"))?
        .json()
        .map_err(|error| format!("Failed to parse QQ Music search response: {error}"))?;

    let expected_title = normalized_media_text(title);
    let expected_artist = normalized_media_text(artist);
    let song_id = search
        ...
        .and_then(|songs| {
            songs.iter().find_map(|song| {
                ...
                let song_artist = song
                    ...
                    .unwrap_or_default();
                let title_matches = song_title == expected_title
                    || song_title.contains(&expected_title)
                    || expected_title.contains(&song_title);
                let artist_matches = expected_artist.is_empty()
                    || song_artist.contains(&expected_artist)
                    || expected_artist.contains(&song_artist);

                if title_matches && artist_matches {
                    song.get("songid").and_then(Value::as_i64)
                } else {
                    None
                }
            })
    ...
    .map_err(|error| format!("Failed to create QQ Music lyric URL: {error}"))?;
    let lyric: Value = client
        .get(lyric_url)
        .header("Referer", "https://y.qq.com/")
        .send()
        .and_then(|response| response.error_for_status())
        .map_err(|error| format!("QQ Music lyric request failed: {error}"))?
        .json()
        .map_err(|error| format!("Failed to parse QQ Music lyric response: {error}"))?;
...
pub fn run() {
    tauri::Builder::default()
        ...
        .setup(|app| {
            notification_listener::start(app.handle().clone());
            notification_listener::start_external_server(app.handle().clone());
            build_tray(app)?;
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            clipboard_history::copy_clipboard_history_item_and_paste_back,

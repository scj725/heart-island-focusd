...
fn read_media_state_from_manager(manager: &MediaSessionManager) -> Result<MediaState, String> {
    ...
    let media_properties = session
        ...
        .ok()
        .and_then(|operation| pollster::block_on(operation).ok());
    let track_title = media_properties
        .as_ref()
        .and_then(|properties| properties.Title().ok())
        .map(|value| value.to_string())
        .unwrap_or_default();
    let track_artist = media_properties
        .as_ref()
        .and_then(|properties| properties.Artist().ok())
        .map(|value| value.to_string())
        .unwrap_or_default();
    let playback_position_seconds = session
        .GetTimelineProperties()
        .ok()
        .and_then(|timeline| timeline.Position().ok())
        .map(|position| position.Duration as f64 / 10_000_000.0)
        .unwrap_or(0.0);
